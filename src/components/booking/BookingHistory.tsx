...
export default function BookingHistory({ userId, userType }: BookingHistoryProps) {
  ...
  const getStatusBadge = (status: BookingStatus) => {
    switch (status) {
      ...
      default:
        return <Badge className="bg-muted/50 text-foreground/70">{status}</Badge>;
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {bookings.map((booking) => (
          <div
            key={booking.id}
            className="bg-muted/50 backdrop-blur-md border border-secondary/20 rounded-xl overflow-hidden"
          >
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  {/* Date Badge */}
                  <div className="hidden sm:flex flex-col items-center bg-card rounded-lg p-2 min-w-[50px]">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="font-semibold text-secondary flex items-center gap-0.5 justify-end">
                      <IndianRupee className="h-4 w-4" />
                      {booking.fare}
                    </p>
                    <p className="text-xs text-foreground/50">{booking.distance} km</p>
                  </div>
            ...
            {/* Expanded Details */}
            {expandedBooking === booking.id && (
              <div className="px-4 pb-4 pt-2 border-t border-white/10 bg-card/50">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Route Details */}
                  <div className="space-y-3">
                    <h4 className="font-medium text-sm text-secondary">Route Details</h4>
                    <div className="space-y-2">
                      <div className="flex items-start gap-2">
                        <div className="w-2 h-2 rounded-full bg-primary mt-1.5" />
                        <div>
                          <p className="text-xs text-foreground/50">Pickup</p>
                          <p className="text-sm text-foreground">{booking.pickupLocation.address || 'Pickup Location'}</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-2">
                        <div className="w-2 h-2 rounded-full border-2 border-red-500 mt-1.5" />
                        <div>
                          <p className="text-xs text-foreground/50">Drop</p>
                          <p className="text-sm text-foreground">{booking.dropLocation.address || 'Drop Location'}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Time Details */}
                  <div className="space-y-3">
                    <h4 className="font-medium text-sm text-secondary">Time Details</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-foreground/50">Booked at</span>
                        <span className="text-foreground">{formatTime(booking.bookingTime)}</span>
                      </div>
                      {booking.rideStartTime && (
                        <div className="flex justify-between">
                          <span className="text-foreground/50">Started at</span>
                          <span className="text-foreground">{formatTime(booking.rideStartTime)}</span>
                        </div>
                      )}
                      {booking.rideEndTime && (
                        <div className="flex justify-between">
                          <span className="text-foreground/50">Ended at</span>
                          <span className="text-foreground">{formatTime(booking.rideEndTime)}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      {userType === 'student' && booking.driverRating && (
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-foreground/50">Your rating:</span>
                      ...
                      {userType === 'driver' && booking.studentRating && (
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-foreground/50">Student rating:</span>
                          <div className="flex items-center gap-0.5">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <Star
                                key={star}
                    ...
                    {booking.driverReview && userType === 'student' && (
                      <p className="text-sm mt-2 italic text-foreground/60">
                ...
                <div className="mt-4 pt-4 border-t border-white/10">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-foreground">{booking.vehicleNumber}</span>
                  </div>
                </div>
              </div>
            )}
